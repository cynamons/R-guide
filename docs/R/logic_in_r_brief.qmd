...
In class we looked at subsetting two columns of the data, Professor.csv. But say we wanted to combine those to statements to look at males with a salary over 50,000 and females with a salary less than or equal to 50,000. 
...
With the parenthesis in place, hopefully this better reinforces the order that Gender and Salary arguments could be swapped and even the Male to Female arguments around the | could be swapped, but they cannot be mixed around without confusing which Salary condition applies to which Gender.
