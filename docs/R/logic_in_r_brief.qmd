...
Someone in STA-6443 Evening class asked if the order of the logical statements inside of a data.frame subsetting function impacted the outcome. In the scenario provided, the subset was evaluating 2 logical statements, and we were examining the difference of "&" and "|".
...
text_tbl <- data.frame(
  ...
  Description = c(
    ...
    "similar to if(conf) but may be preferable because also checks for NAs",
...
Plain English: when given two lists to compare it will do that for each paired item in that list
...
```
Both statements return TRUE, but the intent of the dev was that the second line would evaluate to FALSE. Or in the case of our example from class we may want to be very specific with how we subset data.
...
One of the other ways we can influence the precedence is by inserting parenthesis to override the AND vs OR rule and force a relationship to evaluate first.

## Back to the original data, and subsetting a data.frame
In class we looked at subsetting two columns of the data, prof. But say we wanted to combine those to statements to look at males with a salary over 50,000 and females with a salary less than or equal to 50,000. 
...
prof.appended = rbind(prof.female.low, prof.male.high)
dim(prof.appended)
```
...
In this scenario we do not need any parenthesis, because our focus is on the relation ships between Gender and Salary. Order, in this case matters, but still not around the & operator (similar to the original explanation in class).
...
With the parenthesis in place, hopefully this better reinforces the order that Gender and Salary arguments could be swapped and even the Male to Female arguments around the Or could be swapped, but they cannot be mixed around without confusing which Salary applies to which Gender.
